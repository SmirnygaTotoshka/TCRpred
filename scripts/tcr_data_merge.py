...
def epitope(keep, output, files):
    ...
    alpha_human = merged_data.query("cdr3_alpha.notna() and host_species == 'human'").rename(columns = {'cdr3_alpha':'Structure', 'epitope': 'Activity'})
    ...
    beta_human = merged_data.query("cdr3_beta.notna() and host_species == 'human'").rename(columns = {'cdr3_beta':'Structure', 'epitope': 'Activity'})
    ...
    alpha_mouse = merged_data.query("cdr3_alpha.notna() and host_species == 'mouse'").rename(columns = {'cdr3_alpha':'Structure', 'epitope': 'Activity'})
    ...
    beta_mouse = merged_data.query("cdr3_beta.notna() and host_species == 'mouse'").rename(columns = {'cdr3_beta':'Structure', 'epitope': 'Activity'})
...
def mhc(keep, output, files):
    ...
    alpha_human = merged_data.query("cdr3_alpha.notna() and host_species == 'human'").rename(columns = {'cdr3_alpha':'Structure'})
    alpha_human['chain'] = 'alpha'
    print(f'Alpha_human {alpha_human.shape}')
    alpha_human.to_csv(os.path.join(output, "cdr3alpha_mhc_human.csv"), sep = ";", index=False)
    
    beta_human = merged_data.query("cdr3_beta.notna() and host_species == 'human'").rename(columns = {'cdr3_beta':'Structure'})
    beta_human['chain'] = 'beta'
    print(f'Beta_human {beta_human.shape}')
    beta_human.to_csv(os.path.join(output, "cdr3beta_mhc_human.csv"), sep = ";", index=False)
    
    alpha_mouse = merged_data.query("cdr3_alpha.notna() and host_species == 'mouse'").rename(columns = {'cdr3_alpha':'Structure'})
    alpha_mouse['chain'] = 'alpha'
    print(f'Alpha_mouse {alpha_mouse.shape}')
    alpha_mouse.to_csv(os.path.join(output, "cdr3alpha_mhc_mouse.csv"), sep = ";", index=False)
    
    beta_mouse = merged_data.query("cdr3_beta.notna() and host_species == 'mouse'").rename(columns = {'cdr3_beta':'Structure'})
    beta_mouse['chain'] = 'beta'
    print(f'Beta_mouse {beta_mouse.shape}')
